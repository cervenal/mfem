...
SparseMatrix* GetEdgeConstraint(FiniteElementSpace &fespace, int edge, int edge_order)
{
   ...
   SparseMatrix *cR = new SparseMatrix(n_true_dofs, ndofs);

   // Put identity in the prolongation and restriction matrices for true DOFs (not the virtual ones)
   int true_dof = 0;
   for (int i = 0; i < ndofs; i++)
   {
   ...
   for (unsigned mi = 0; mi < list.masters.size(); mi++)
   {
      if (is_slave_master[mi])
      {
         ...
         // Compute pseudoinverse (left inverse) of the interpolation matrix I
         DenseMatrix PI(I.Width(),I.Height());
         DenseMatrix IT;
         DenseMatrix ITI(I.Width());
         IT.Transpose(I);
         Mult(IT,I,ITI);
         ITI.Invert();
         Mult(ITI,IT,PI);
         ...
         // Matrix R: Add dependency of virtual dofs on vertex dofs of the master edge
         for (int i = 0; i < 2; i++)
         {
           for (int j = 2; j < n_virtual; j++)
           {
             if (std::abs(PI(j, i)) > 1e-12)
             {
               cR->Add(true_dof + (j-2), master_dofs[i], PI(j, i));
             }
           }
         }
         ...
         for (int i = 2; i < master_dofs.Size(); i++)
         {
            ...
            for (int j = 2; j < n_virtual; j++)
            {
               // Matrix R: Add dependency of virtual dofs on inner edge dofs of the master edge
               if (std::abs(PI(j, i)) > 1e-12)
               {
                 cR->Add(true_dof + (j-2), master_dofs[i], PI(j, i));
               }
